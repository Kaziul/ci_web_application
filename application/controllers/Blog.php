...
class Blog extends CI_Controller {
	public function index($page = 1)
	{
		...
		$param['limit'] = ($page * $perpage) - $perpage;// (1 *10) -10 = 0
	...
	public function categories()
	{
		...
		$categories = $this->Category_model->getCatagoriesFront();
		$data = [];
		$data['categories'] = $categories;
		$this->load->view('front/categoris',$data);
	}
	...
	public function category($category_id, $page = 1)
	{
		...
		$this->load->library('pagination');
		$this->load->helper('text');
		$category = $this->Category_model->getCatagory($category_id);
		if(empty($category)){
			redirect(base_url('blog'));
		}
